...
function descargarCalendario() {
  const ics = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//Henry y Joselyn//Boda//ES',
    'BEGIN:VEVENT',
    'UID:ceremonia-henry-joselyn@boda',
    'DTSTART:20270109T193000Z',
    'DTEND:20270109T203000Z',
    'SUMMARY:Ceremonia religiosa — Henry & Joselyn',
    'LOCATION:Iglesia Nuestra Señora del Perpetuo Socorro\\, Sabana Sur\\, San José\\, Costa Rica',
    'DESCRIPTION:Ceremonia religiosa de la boda de Henry y Joselyn. Llegada 1:30 p.m.',
    'END:VEVENT',
    'BEGIN:VEVENT',
    'UID:recepcion-henry-joselyn@boda',
    'DTSTART:20270109T220000Z',
    'DTEND:20270110T040000Z',
    'SUMMARY:Recepción — Henry & Joselyn',
    'LOCATION:Vista Capital\\, Alajuelita\\, San José\\, Costa Rica',
    'DESCRIPTION:Recepción de la boda de Henry y Joselyn. Inicio 4:00 p.m.',
    'END:VEVENT',
    'END:VCALENDAR',
  ].join('\r\n');
  ...
  a.href = url;
  a.download = 'boda-henry-joselyn.ics';
...
export default function Invitation() {
  ...
  const [alcoholCantidad, setAlcoholCantidad] = useState(0);
  ...
  useEffect(() => {
    async function load() {
      ...
      if (error || !data) {
        ...
        setGuest(data);
        setCantidad(data.rsvp_cantidad_confirmada || data.cupo);
        setAlcoholCantidad(data.rsvp_alcohol_cantidad || 0);
        setRestricciones(data.rsvp_restricciones || '');
  ...
  function openCard() {
    ...
    const timing = reduceMotion
      ...
      : { reveal: 500, bloom: 700, fade: 900, remove: 1200 };

    if (ov) {
      ov.classList.add('is-opening');
      // Se desactiva el bloqueo de toques DE INMEDIATO al tocar, sin esperar
      // a que termine ninguna animación — así nunca puede "atajar" el scroll.
  ...
  async function enviarRSVP(resp) {
    ...
    const alcoholFinal = resp === 'si' ? alcoholCantidad : 0;
    const { error } = await supabase
      .from('invitados')
      .update({
        rsvp_estado: resp,
        rsvp_cantidad_confirmada: cantidadFinal,
        rsvp_alcohol_cantidad: alcoholFinal,
        rsvp_restricciones: resp === 'si' ? (restricciones.trim() || null) : null,
  ...
  return (
    ...
    >
      ...
      <section data-scene="" data-screen-label="Hero" style={{ position: 'relative', height: '100svh', minHeight: 600, overflow: 'hidden', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', background: '#1a1f1c' }}>
        <img src={foto01} alt="El castillo" fetchPriority="high" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 34%', animation: 'kb 24s ease-in-out infinite alternate', transformOrigin: '50% 30%' }} />
      ...
      <section data-scene="" data-screen-label="El paseo" style={{ position: 'relative', height: '100svh', minHeight: 600, overflow: 'hidden', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', background: '#1a1f1c' }}>
        <img src={foto02} alt="Henry y Joselyn caminando tomados de la mano" loading="lazy" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 62%', animation: 'kb 22s ease-in-out infinite alternate', transformOrigin: '50% 62%' }} />
      ...
      <section data-scene="" data-screen-label="La propuesta" style={{ position: 'relative', height: '100svh', minHeight: 600, overflow: 'hidden', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', background: '#1a1f1c' }}>
        <img src={foto03} alt="Henry arrodillado frente al castillo" loading="lazy" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 62%', animation: 'kb 20s ease-in-out infinite alternate', transformOrigin: '50% 62%' }} />
      ...
      <section data-scene="" data-screen-label="La pregunta" style={{ position: 'relative', height: '100svh', minHeight: 600, overflow: 'hidden', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', background: '#1a1f1c' }}>
        <img src={foto04} alt="Henry arrodillado frente a Joselyn durante la propuesta" loading="lazy" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 32%', animation: 'kbzoom 18s ease-in-out infinite alternate', transformOrigin: '62% 42%' }} />
      ...
      <section style={{ padding: '15vh 30px', textAlign: 'center', background: 'linear-gradient(180deg,#F1EEF6,#e9ece4)' }}>
        <div data-reveal="" style={{ maxWidth: 380, margin: '0 auto' }}>
          <p style={{ fontFamily: "'Playfair Display',serif", fontStyle: 'italic', fontWeight: 400, fontSize: 'clamp(22px,6vw,28px)', lineHeight: 1.5, color: '#55496a', margin: 0 }}>El tiempo se quedó en silencio, esperando la respuesta.</p>
        </div>
      </section>
      ...
      <section data-scene="" data-screen-label="El anillo" style={{ position: 'relative', height: '100svh', minHeight: 600, overflow: 'hidden', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', background: '#1a1f1c' }}>
        <img src={foto05} alt="Henry colocando el anillo en el dedo de Joselyn" loading="lazy" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 42%', animation: 'kbzoom 18s ease-in-out infinite alternate', transformOrigin: '50% 46%' }} />
        ...
        <div data-cap="" style={{ position: 'relative', zIndex: 3, textAlign: 'center', color: '#fff', padding: '0 28px 13vh', maxWidth: 440 }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 500, fontSize: 'clamp(28px,8vw,42px)', lineHeight: 1.15, margin: '0 0 6px', textShadow: '0 3px 28px rgba(8,12,9,.95)' }}>Ahí quedó sellado lo que ya sentíamos.</h2>
      ...
      <section data-scene="" data-screen-label="La promesa" style={{ position: 'relative', height: '100svh', minHeight: 600, overflow: 'hidden', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', background: '#1a1f1c' }}>
        <img src={foto06} alt="Joselyn con el anillo, abrazando a Henry" loading="lazy" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 40%', animation: 'kb 19s ease-in-out infinite alternate', transformOrigin: '55% 40%' }} />
        ...
        <div data-cap="" style={{ position: 'relative', zIndex: 3, textAlign: 'center', color: '#fff', padding: '14vh 28px 0', maxWidth: 440 }}>
          <p style={{ fontFamily: "'Playfair Display',serif", fontStyle: 'italic', fontWeight: 400, fontSize: 'clamp(23px,6.6vw,32px)', lineHeight: 1.4, margin: 0, textShadow: '0 3px 28px rgba(8,12,9,.95)' }}>Ahí comenzó la cuenta regresiva hacia esta nueva vida.</p>
        </div>
      </section>
      ...
      <section data-scene="" data-screen-label="Dije que sí" style={{ position: 'relative', height: '100svh', minHeight: 600, overflow: 'hidden', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', background: '#1a1f1c' }}>
        <img src={foto07} alt="Henry y Joselyn celebrando con el mensaje I Said Yes" loading="lazy" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 52%' }} />
      ...
      <section data-scene="" data-screen-label="El beso" style={{ position: 'relative', height: '100svh', minHeight: 600, overflow: 'hidden', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', background: '#1a1f1c' }}>
        <img src={foto08} alt="Henry y Joselyn besándose bajo el arco de rosas" loading="lazy" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 16%', animation: 'kb 20s ease-in-out infinite alternate', transformOrigin: '50% 20%' }} />
      ...
      <section data-screen-label="Nos casamos" style={{ position: 'relative', minHeight: '100svh', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '14vh 28px', background: 'linear-gradient(160deg,#EDE6F4 0%,#EAEFE8 55%,#E7EFEA 100%)' }}>
        <div data-reveal="" style={{ textAlign: 'center', maxWidth: 460 }}>
          ...
          <button
            ...
            style={{ marginTop: 26, padding: '12px 26px', border: '1px solid #9a86b8', borderRadius: 24, background: '#fff', color: '#7d6aa8', fontFamily: "'Mulish'", fontWeight: 700, fontSize: 12, letterSpacing: 1, textTransform: 'uppercase', cursor: 'pointer' }}
          >
      ...
      <section data-screen-label="Detalles" style={{ padding: '14vh 28px 8vh', background: '#e9ece4' }}>
        <div data-reveal="" style={{ maxWidth: 440, margin: '0 auto 40px' }}>
          <div style={{ height: 420, borderRadius: '200px 200px 20px 20px', overflow: 'hidden', boxShadow: '0 26px 54px -30px rgba(90,110,95,.6)' }}>
            <img src={foto09} alt="Henry y Joselyn abrazados frente al castillo" loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 42%' }} />
          </div>
        </div>
        ...
        <div style={{ maxWidth: 440, margin: '0 auto', display: 'grid', gap: 18 }}>
          <div data-reveal="" style={{ background: '#fff', borderRadius: 20, padding: '32px 24px', textAlign: 'center', boxShadow: '0 18px 40px -30px rgba(90,110,95,.5)' }}>
            ...
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 25, color: '#40384a', margin: '8px 0 4px' }}>1:30 p.m.</div>
      ...
      <section style={{ padding: '2vh 28px 10vh', background: '#e9ece4' }}>
        <div data-reveal="" style={{ maxWidth: 440, margin: '0 auto', padding: '40px 28px', background: 'linear-gradient(135deg,#EDE6F4,#E7EFEA)', borderRadius: 22, textAlign: 'center' }}>
          ...
          <div style={{ fontFamily: "'Mulish'", fontWeight: 300, fontSize: 14, color: '#5f5870', lineHeight: 1.6, textAlign: 'left' }}>
            <p style={{ margin: '0 0 12px' }}>
              <strong style={{ color: '#40384a', fontWeight: 700 }}>Ellas:</strong> las invitamos a lucir cualquier vestimenta formal en tonos pastel (vestido, traje o saco). Solo pedimos evitar el{' '}
              <em style={{ color: '#6E8B74', fontStyle: 'italic', fontWeight: 600 }}>blanco</em> — reservado para la novia — y cualquier tono de{' '}
              <em style={{ color: '#6E8B74', fontStyle: 'italic', fontWeight: 600 }}>verde</em>.
            </p>
            <p style={{ margin: 0 }}>
              <strong style={{ color: '#40384a', fontWeight: 700 }}>Ellos:</strong> saco en negro, azul, gris o vino, combinado con camisa en tonos pastel o blanca. Solo pedimos evitar camisas o trajes en tonos de{' '}
              <em style={{ color: '#6E8B74', fontStyle: 'italic', fontWeight: 600 }}>verde</em>.
            </p>
          </div>
        </div>
      ...
      {/* INFORMACIÓN ÚTIL */}
      <section style={{ padding: '0 28px 8vh', background: '#e9ece4' }}>
        <div data-reveal="" style={{ maxWidth: 400, margin: '0 auto', padding: '26px 24px', background: '#fff', borderRadius: 18, boxShadow: '0 10px 26px -20px rgba(90,110,95,.4)' }}>
          <div style={{ fontFamily: "'Mulish'", fontWeight: 700, fontSize: 12, letterSpacing: 2, textTransform: 'uppercase', color: '#6E8B74', marginBottom: 12, textAlign: 'center' }}>Información útil</div>
          <ul style={{ margin: 0, padding: 0, listStyle: 'none', fontFamily: "'Mulish'", fontSize: 14, color: '#5f5870', lineHeight: 2 }}>
            <li>🅿️ Contamos con parqueo</li>
            <li>🛡️ Parqueo con seguridad 24/7</li>
            <li>
              💬 Si tienes dudas, puedes{' '}
              <a href="https://wa.me/50684239235" target="_blank" rel="noreferrer" style={{ color: '#7d6aa8', fontWeight: 700, textDecoration: 'none' }}>
        ...
        <div data-reveal="" style={{ margin: '0 auto', maxWidth: 400 }}>
          {done ? (
            <div style={{ padding: '40px 26px', borderRadius: 22, background: '#fff', textAlign: 'center', boxShadow: '0 20px 46px -30px rgba(90,110,95,.5)' }}>
              ...
              {choice === 'si' && (
                <div style={{ marginTop: 22, padding: '18px 16px', borderRadius: 14, background: '#f6f3fa', textAlign: 'left', fontFamily: "'Mulish'", fontSize: 13, color: '#5f5870', lineHeight: 1.8 }}>
                  <div><strong style={{ color: '#40384a' }}>Asistencia:</strong> confirmada</div>
                  <div><strong style={{ color: '#40384a' }}>Cantidad:</strong> {cantidad} persona{cantidad > 1 ? 's' : ''}</div>
                  <div><strong style={{ color: '#40384a' }}>Alcohol:</strong> {alcoholCantidad === 0 ? 'ninguno' : `${alcoholCantidad} persona${alcoholCantidad > 1 ? 's' : ''}`}</div>
                  <div><strong style={{ color: '#40384a' }}>Restricciones:</strong> {restricciones.trim() || 'ninguna'}</div>
                </div>
              )}

              <button
                onClick={() => setDone(false)}
                style={{ marginTop: 20, background: 'none', border: 'none', color: '#9a86b8', fontFamily: "'Mulish'", fontWeight: 700, fontSize: 12, letterSpacing: 1, textTransform: 'uppercase', textDecoration: 'underline', cursor: 'pointer' }}
              >
                Modificar respuesta
              </button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {guest.cupo > 1 && (
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setCantidad(n);
                    if (alcoholCantidad > n) setAlcoholCantidad(n);
                  }}
                >
              ...
              <label style={{ fontFamily: "'Mulish'", fontSize: 12, color: '#6f6878', textAlign: 'left' }}>
                ¿Cuántos de ustedes consumen alcohol?
                <select
                  className="rsvp-select"
                  value={alcoholCantidad}
                  onChange={(e) => setAlcoholCantidad(Number(e.target.value))}
                  style={{ width: '100%', marginTop: 6 }}
                >
                  {Array.from({ length: cantidad + 1 }, (_, i) => i).map((n) => (
                    <option key={n} value={n}>{n === 0 ? 'Ninguno' : `${n} persona${n > 1 ? 's' : ''}`}</option>
                  ))}
                </select>
              </label>
              <label style={{ fontFamily: "'Mulish'", fontSize: 12, color: '#6f6878', textAlign: 'left' }}>
                ...
                <textarea
                  ...
                  placeholder="Ej: alergia a mariscos, vegetariano, sin gluten..."
                  rows={2}
                  className="rsvp-select"
                  style={{ width: '100%', marginTop: 6, resize: 'vertical', fontFamily: "'Mulish'" }}
                />
              </label>
              <button
      ...
      <section data-scene="" style={{ position: 'relative', height: '94svh', minHeight: 540, overflow: 'hidden', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', background: '#1a1f1c' }}>
        <img src={foto10} alt="Henry y Joselyn celebrando frente al castillo" loading="lazy" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 50%', animation: 'kb 21s ease-in-out infinite alternate', transformOrigin: '50% 50%' }} />
