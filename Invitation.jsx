...
export default function Invitation() {
  ...
  function setupMotion() {
    ...
    root.querySelectorAll('[data-scene]').forEach((scene) => {
      ...
      if (!img || img.classList.contains('scene-primary')) return;
      img.classList.add('scene-primary');
    ...
    root.querySelectorAll('[data-scene]').forEach((sec) => {
      for (let i = 0; i < 2; i++) {
        const sm = document.createElement('div');
        const left = [8, 66][i];
        const w = [58, 54][i];
        sm.style.cssText =
          'position:absolute;z-index:2;bottom:-60px;left:' + left + '%;width:' + w +
          '%;height:240px;pointer-events:none;background:radial-gradient(ellipse at center,rgba(240,238,246,.32),rgba(230,232,240,0) 70%);filter:blur(24px);animation:smoke ' +
          (13 + i * 3) + 's ease-in-out ' + (i * 2.4) + 's infinite;';
        sec.appendChild(sm);
      }
    });
    ...
    const ambientSparkles = window.innerWidth <= 520 ? 16 : 26;
    for (let i = 0; i < ambientSparkles; i++) {
      const sp = document.createElement('div');
      const sz = (Math.random() * 2.4 + 1.3).toFixed(1);
      const dur = (Math.random() * 4 + 3.5).toFixed(1);
    ...
    const caps = Array.from(root.querySelectorAll('[data-cap]'));
    const cinematicScenes = Array.from(root.querySelectorAll('[data-scene]'));
    let ticking = false;
    const apply = () => {
      ...
      caps.forEach((el) => {
        ...
        const d = (mid - vh / 2) / vh;
        const focusOpacity = 1 - Math.min(1, Math.abs(d) * 0.92);
        const o = 0.42 + focusOpacity * 0.58;
        el.style.opacity = o.toFixed(3);
        el.style.transform = 'translate3d(0,' + (d * 18).toFixed(1) + 'px,' + (-Math.abs(d) * 12).toFixed(1) + 'px)';
      });
      cinematicScenes.forEach((sec) => {
        const r = sec.getBoundingClientRect();
        ...
        const primary = sec.querySelector('.scene-primary');
        if (!primary) return;
        let strength = 0.028, x = (progress - 0.5) * -1.2, y = (progress - 0.5) * -1.8, origin = '50% 50%';
        if (label === 'La propuesta') { strength = 0.045; x = focus * -0.8; y = focus * 1.2; origin = '48% 72%'; }
        if (label === 'La pregunta') { strength = 0.038; x = focus * 0.7; y = focus * -0.5; origin = '54% 42%'; }
        if (label === 'El anillo') { strength = 0.05; x = focus * -0.6; y = focus * 0.25; origin = '53% 47%'; }
        if (label === 'La promesa') { strength = 0.035; x = focus * 0.6; y = focus * -0.35; origin = '55% 46%'; }
        if (label === 'Dije que sí') { strength = 0.025; x = focus * -0.3; y = 0; origin = '46% 54%'; }
        if (label === 'El paseo') { strength = 0.02; x = (progress - 0.5) * 1.1; origin = '50% 72%'; }
        const scale = 1.008 + focus * strength;
        primary.style.transformOrigin = origin;
        primary.style.transform = 'translate3d(' + x.toFixed(2) + '%,' + y.toFixed(2) + '%,0) scale(' + scale.toFixed(4) + ')';
      });
      ticking = false;
    };
  ...
  function openCard() {
    ...
    const timing = reduceMotion
      ...
      : { reveal: 1180, bloom: 1320, fade: 2420, remove: 2940 };
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Cormorant Garamond', serif", fontSize: 20, color: '#5f7a67' }}>
        Cargando invitación…
      </div>
    );
  }
  if (notFound) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: 24, fontFamily: "'Cormorant Garamond', serif", fontSize: 20, color: '#5f7a67' }}>
  ...
  return (
    ...
    >
      ...
      {/* LA PREGUNTA */}
      <section data-scene="" data-screen-label="La pregunta" style={{ position: 'relative', height: '100svh', minHeight: 600, overflow: 'hidden', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', background: '#1a1f1c' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, zIndex: 2, background: 'linear-gradient(to bottom, rgba(20,24,21,.9) 0%, rgba(20,24,21,.5) 26%, rgba(20,24,21,0) 58%, rgba(20,24,21,.35) 100%)' }}></div>
        <div data-cap="" style={{ position: 'relative', zIndex: 3, textAlign: 'center', color: '#fff', padding: '14vh 26px 0', maxWidth: 440 }}>
          <div style={{ fontFamily: "'Great Vibes',cursive", fontSize: 'clamp(42px,14vw,60px)', lineHeight: 1.12, color: '#fff', textShadow: '0 4px 30px rgba(8,12,9,.95)' }}>Y entonces llegó la pregunta que cambiaría nuestros planes para siempre.</div>
        </div>
      </section>
      ...
      {/* DIJE QUE SÍ */}
      <section data-scene="" data-screen-label="Dije que sí" style={{ position: 'relative', height: '100svh', minHeight: 600, overflow: 'hidden', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', background: '#1a1f1c' }}>
        <img src={foto07} alt="Henry y Joselyn celebrando con el mensaje I Said Yes" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 52%', animation: 'kb 23s ease-in-out infinite alternate', transformOrigin: '50% 52%' }} />
        <div style={{ position: 'absolute', inset: 0, zIndex: 2, background: 'linear-gradient(to bottom, rgba(20,24,21,.9) 0%, rgba(20,24,21,.45) 24%, rgba(20,24,21,0) 52%, rgba(20,24,21,.3) 100%)' }}></div>
        <div data-cap="" style={{ position: 'relative', zIndex: 3, textAlign: 'center', color: '#fff', padding: '14vh 28px 0', maxWidth: 440 }}>
          <p style={{ fontFamily: "'Playfair Display',serif", fontStyle: 'italic', fontWeight: 400, fontSize: 'clamp(22px,6.4vw,30px)', lineHeight: 1.4, margin: 0, textShadow: '0 3px 28px rgba(8,12,9,.95)' }}>Del sí de aquel día al sí para toda la vida.</p>
        </div>
      </section>
      ...
      <section style={{ padding: '16vh 30px', textAlign: 'center', background: '#e9ece4' }}>
        <div data-reveal="" style={{ maxWidth: 440, margin: '0 auto' }}>
          <span style={{ display: 'inline-block', fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontSize: 40, color: '#9a86b8', lineHeight: 0, marginBottom: 6 }}>❦</span>
          <p style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontWeight: 500, fontSize: 'clamp(24px,6.6vw,32px)', lineHeight: 1.45, color: '#40384a', margin: '8px 0 14px' }}>«Por encima de todo, revístanse del amor, que es el vínculo perfecto.»</p>
      ...
      <section style={{ padding: '2vh 28px 10vh', background: '#e9ece4' }}>
        <div data-reveal="" style={{ maxWidth: 440, margin: '0 auto', padding: '40px 28px', background: 'linear-gradient(135deg,#EDE6F4,#E7EFEA)', borderRadius: 22, textAlign: 'center' }}>
          ...
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(24px,7vw,30px)', color: '#40384a', margin: '8px 0 4px' }}>Formal <span style={{ fontSize: '.55em', color: '#9a86b8', padding: '0 .25em' }}>✦</span> tonos pastel sugeridos</div>
          ...
          <div style={{ fontFamily: "'Mulish'", fontWeight: 300, fontSize: 14, color: '#5f5870', lineHeight: 1.55 }}>
            El código es formal. Los tonos pastel son una sugerencia, no una obligación. Con cariño te pedimos evitar{' '}
      ...
      {/* PERSONAL */}
      <section style={{ padding: '16vh 30px', textAlign: 'center', background: 'linear-gradient(180deg,#eef0e9,#e9ece4)' }}>
        <div data-reveal="" style={{ maxWidth: 440, margin: '0 auto' }}>
          <div style={{ fontFamily: "'Great Vibes',cursive", fontSize: 'clamp(30px,8vw,42px)', color: '#9a86b8' }}>Gracias por acompañarnos</div>
          <p style={{ fontFamily: "'Playfair Display',serif", fontStyle: 'italic', fontWeight: 400, fontSize: 'clamp(22px,6vw,29px)', lineHeight: 1.5, color: '#40384a', margin: '12px 0 0' }}>Nos hará muy felices compartir este día con quienes han sido parte de nuestra historia.</p>
        </div>
      </section>
      ...
      <section data-screen-label="RSVP" style={{ padding: '2vh 28px 13vh', background: '#e9ece4' }}>
        <div data-reveal="" style={{ textAlign: 'center', maxWidth: 440, margin: '0 auto 26px' }}>
          <span style={{ fontFamily: "'Great Vibes',cursive", fontSize: 34, color: '#9a86b8' }}>Acompáñanos</span>
        ...
        <div data-reveal="" style={{ margin: '0 auto', maxWidth: 400 }}>
          {done ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div className="rsvp-guest-name">{guest.mensaje_personalizado || guest.nombre_familia}</div>
      ...
      <section data-scene="" style={{ position: 'relative', height: '94svh', minHeight: 540, overflow: 'hidden', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', background: '#1a1f1c' }}>
        ...
        <div data-cap="" style={{ position: 'relative', zIndex: 3, textAlign: 'center', color: '#fff', padding: '13vh 28px 0' }}>
          ...
          <div style={{ fontFamily: "'Mulish'", fontWeight: 500, fontSize: 11, letterSpacing: 3, textTransform: 'uppercase', color: 'rgba(255,255,255,.9)', marginTop: 16 }}>Gracias por ser parte de nuestra historia</div>
        </div>
      </section>
    </div>
  );
